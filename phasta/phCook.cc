...
namespace ph {
  ...
  void preprocess(apf::Mesh2* m, Input& in, Output& out, BCs& bcs) {
    ...
    if(!PCU_Comm_Self()) lion_oprint(1,"Output generate successful\n");
    ...
    ph::writeGeomBC(out, subDirPath); //write geombc
      if(!PCU_Comm_Self()) lion_oprint(1,"GeombBC files written successfully\n");
    if(!PCU_Comm_Self())
      ph::writeAuxiliaryFiles(path, in.timeStepNumber);
    m->verify();
    if(!PCU_Comm_Self()) lion_oprint(1,"Mesh verified\n");
#ifdef HAVE_SIMMETRIX
    gmi_model* g = m->getModel();
    if(!PCU_Comm_Self()) lion_oprint(1,"Model loaded\n");
    ph::clearAttAssociation(g,in);
    if(!PCU_Comm_Self()) lion_oprint(1,"Mesh cleared Association\n");
#endif
    if (in.adaptFlag)
      ph::goToParentDir();
    if(in.printIOtime) phastaio_printStats();
    if(!PCU_Comm_Self()) lion_oprint(1,"Stats printed\n");
  }
  void preprocess(apf::Mesh2* m, Input& in, Output& out) {
    gmi_model* g = m->getModel();
    PCU_ALWAYS_ASSERT(g);
    if(!PCU_Comm_Self()) lion_oprint(1,"Model Asserted\n");
    BCs bcs;
