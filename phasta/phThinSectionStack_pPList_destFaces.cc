...
//    IF gFace has extrusion/thin stack mesh attribute
        ...
        if(EN_isExtrusionEntity(face) == 1){
		  ...
		  for(int j = 0; j < 1; j++){
			if((EN_isExtrusionEntity(F_region(face, j))) == 1){
				...
				PList_app(gDestFaces, F_whatIn(PList_item(faces_stack(PList_size(faces_stack)))));
	...
	for(int i = 0; i < PList_size(gDestFaces); i++){
      for(int j = 0; j < PList_size(gDestFaces); j++){
	    if(PList_item(gDestFaces,i) == PList_item(gDestFaces,j));
			destFace = (pGFace)PList_item(gDestFaces,i);
			PList_remItem(gSrcFaces, PList_item(gDestFaces,i));
			break;
	  }
	}
