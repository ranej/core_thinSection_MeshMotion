...
static void getBoundary(Output& o, BCs& bcs, apf::Numbering* n)
{
  ...
  for (int i = 0; i < bs.getSize(); ++i) {
    ...
    js[i] = 0;
    if (m2gb_flag ==1){
      m2gb[i] = new int*[bs.nElements[i]];
      for (int j = 0; j < bs.nElements[i]; ++j)
        m2gb[i][j] = new int[3];
    }
  }
  ...
  while ((f = m->iterate(it))) {
    ...
    /* get m2gb info */
    if (m2gb) {     
      gmi_ent* ge2 = (gmi_ent*)m->toModel(e);
      int rtag = gmi_tag(gm,ge2);
      m2gb[i][j][0] = ftag;
      m2gb[i][j][1] = rtag;
      m2gb[i][j][2] = 0; //look into mesh face usage by mesh region
    }        
    ++js[i];
  }
  m->end(it);
...
static void getGCEssentialBCs(Output& o, apf::Numbering* n)
{
  ...
  if(!in.ensa_melas_dof)
    return;
  std::cout << "getGCBCs_PCU \n";
//  PCU_Comm_Begin();
  std::cout << "getGCBCs_PCU2 \n";
...
// transfer entity to numbering
  o.arrays.iTSlvid = new int[o.nThinSectionStackMeshVertices];
  for(int i = 0; i < o.nThinSectionStackMeshVertices; i++){
    o.arrays.iTSlvid[i] = apf::getNumber(n, o.arrays.iTSlv[i], 0, 0);
  }
}
...
Output::~Output()
{
  ...
  delete [] arrays.nbc;
  delete [] arrays.ibc;
  if (arrays.m2gb) delete [] arrays.m2gb;
